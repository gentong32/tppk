...
<div class="content-wrap">
    ...
    <div style="margin-top: 15px;">
        <?php if ($sebagai == "operatorsatgas" || $statustppk == "ketua" || $statustppk == "anggota1") { ?>
            <button class="btn_ijo" onclick="inputform();">INPUT LAPORAN</button>
        <?php } ?>
        <?php if (($sebagai == "pusat" || $sebagai == "operatorsatgas") && $level == 1) { ?>
            <button class="btn_ijo" onclick="laporan();">STATUS LAPORAN PROVINSI</button>
        <?php } ?>
        <?php if ($sebagai == "pusat" && $level == 5) { ?>
            <button class="btn_ijo" onclick="laporan();">STATUS LAPORAN KAB/KOTA</button>
        <?php } ?>
        <?php if ($sebagai == "pusat" && $indeks == 2) { ?>
            <button class="btn_ijo" onclick="laporan();">LAPORAN DINAS KAB/KOTA</button>
        <?php } ?>
        <?php if ($sebagai == "dinasprovinsi" && $level == 1) { ?>
            <button class="btn_ijo" onclick="statuslaporan();">STATUS LAPORAN PROVINSI</button>
        <?php } ?>
        <?php if ($indeks == 3 || ($sebagai == "pusat" && $level == 2) || ($sebagai != "pusat" && $level == 5)) { ?>
            <button class="btn_ijo" onclick="laporan();">STATUS SATUAN PENDIDIKAN</button>
        <?php } ?>
        <div class="informasi">
            <table class="table table-striped" id="example">
                <thead>
                    <th width="10px">No</th>
                    <th width="220px">Nomor Register</th>
...
<script>
    ...
    function laporan() {
        window.open("<?= base_url('status_laporan_kekerasan') ?>?kode_wilayah=" + kode_wilayah, "_self");
    }
</script>
<?= $this->endSection(); ?>