...
<div class="content-wrap">
        ...
        <?php if ($instansiid == 1) : ?>
            <div class="dwilayah">
                ...
                <div id="dkota" style="display:inline-block;">
                    <select class="combobox1" id="kota" name="kota" onchange="pilkota(this)">
                        <?php
                        $sudahoke = false;
                        if ($sudahoke == true) { ?>
                        ...
                        foreach ($daftar_kota as $item2) { ?>
                            <option <?php echo (substr($kode_wilayah, 0, 6) == substr($item2['kode_wilayah'], 0, 6)) ? 'selected' : ''; ?> value="<?= $item2['kode_wilayah'] ?>"><?= $item2['nama'] ?></option>";
                        <?php }
                        ?>
                    </select>
                </div>
    ...
    <div class="informasi">
        ...
        <span style="color: red;"><i>*) Tidak update semester genap tahun ajaran <?= $tahunajaran ?></i></span>
    </div>
</div>
<?= $this->endSection(); ?>
...
<script>
    ...
    function pilkota(selectElement) {
        document.getElementById('tbunduh1').style.display = "none";
        document.getElementById('tbunduh2').style.display = "none";
    }
    ...
    function filterdata() {
        ...
        <?php if ($instansiid == 1) { ?>
            ...
            window.open("<?= base_url() ?>inputdata/daftarsekolahnontppk?jenjang=" + jenjang + "&kode_wilayah=" + kota, "_self");
        <?php } else { ?>
            window.open("<?= base_url() ?>inputdata/daftarsekolahnontppk?jenjang=" + jenjang, "_self");
        <?php } ?>
